...
- `app.state.reference_portfolio_cache`: In-memory cache for reference portfolios; type: cachetools.FIFOCache(maxsize=5), keyed by (ticker, asset_type), populated on-demand by the reference performance endpoint
- `app.state.reference_portfolio_cache_lock`: Optional threading.Lock for thread-safe access to reference_portfolio_cache (cachetools caches are not thread-safe)
...
- `get_all_positions_endpoint(...) -> PortfolioAllResponse`: GET endpoint at `/portfolio/all` that requires JWT authentication. Supports pagination and sorting via query parameters: `page` (default: 1), `size` (default: 20, max: 100), `sort_by` (default: "ticker"), and `sort_order` (default: "asc"). Verifies token via auth module, retrieves CompositePortfolio from application state, creates PriceService instance, calls portfolio_service.get_all_positions() with sorting parameters to fetch and transform positions, applies pagination using fastapi-pagination, retrieves portfolio totals using composite_portfolio.get_total_cost_basis(), get_total_market_value(), get_total_unrealized_pnl(), and get_total_realized_pnl(), then returns PortfolioAllResponse containing paginated positions and portfolio totals. Validates sort_by against allowed Position fields and returns 400 Bad Request for invalid fields. Logs request/response at INFO level.
...
- `get_asset_trades_all_endpoint(ticker: str, ...) -> PortfolioAssetTradesAllResponse`: GET endpoint at `/portfolio/trades/{ticker}/all` that requires JWT authentication. Supports date filtering and sorting via query parameters: `start_date` (optional, ISO format YYYY-MM-DD), `end_date` (optional, ISO format YYYY-MM-DD), `sort_by` (default: "date"), and `sort_order` (default: "asc"). Does not support pagination - returns all trades. Verifies token via auth module, retrieves CompositePortfolio from application state, calls portfolio_service.get_asset_trades() with ticker, date filtering, and sorting parameters to fetch and transform trades, then returns PortfolioAssetTradesAllResponse containing all trades (no pagination). Validates date formats and date range (start_date <= end_date), validates sort_by against allowed Trade fields, returns 400 Bad Request for invalid dates or sort_by field, 404 Not Found if ticker doesn't exist. Logs request/response at INFO level.
...
- `get_reference_performance_endpoint(ticker: str, ...) -> PortfolioPerformanceResponse`: GET endpoint at `/reference/{ticker}/performance` that requires JWT authentication. Obtains the reference portfolio cache via `get_reference_portfolio_cache()` dependency and passes it (and optional lock) to `portfolio_service.get_reference_portfolio_performance()`. Creates or reuses a reference portfolio (cached by (ticker, asset_type)) using BuyAndHoldStrategy that converts all trades in the original portfolio to trades in a single reference asset defined by the requested `ticker` and `asset_type`, then retrieves historical performance. The reference ticker **does not need** to be present in the imported portfolio; it is used purely as a benchmark for comparison. Supports filtering and configuration via query parameters:
  - `asset_type` (required, string): Asset type for the reference ticker (e.g., "Stock", "ETF", "Crypto"). This controls how the reference `Asset` is constructed and how prices are retrieved. Must be one of a small set of supported values; invalid values result in 400 Bad Request.
  - `start_date` (optional, ISO format YYYY-MM-DD, defaults to portfolio start_date): Start date for performance tracking. Parsed using the same logic as the portfolio performance endpoint.
  - `end_date` (optional, ISO format YYYY-MM-DD, defaults to today): End date for performance tracking.
  - `granularity` (optional, default: "daily", values: "daily", "weekly", "monthly"): Granularity control for history points.
  Verifies token via auth module, retrieves CompositePortfolio and PriceService from application state, parses start_date and end_date using the same logic as the portfolio performance endpoint (including clamping to portfolio start_date and validating start_date <= end_date after clamping), and calls `portfolio_service.get_reference_portfolio_performance()` with composite_portfolio, ticker, asset_type, price_service, start_date, end_date, and reference_portfolio_cache (and optional lock). On cache hit the service uses the cached reference portfolio for get_historical_performance only; on cache miss it creates the reference portfolio, stores it in the cache (FIFO eviction when at max size 5), then retrieves performance. Calls `portfolio_service.apply_granularity_filter()` with the retrieved history points, start_date, end_date, and granularity to filter points based on granularity parameter, then returns PortfolioPerformanceResponse containing the filtered list of PortfolioHistoryPoint objects. For "weekly" granularity, returns history points spaced one week apart starting from the next Monday after start_date (if start_date is not a Monday). For "monthly" granularity, returns history points at the start of each month. Returns 400 Bad Request for invalid date format, invalid date range, invalid granularity value, or invalid `asset_type`, 422 Unprocessable Entity for invalid granularity value (handled by FastAPI Query validation), 500 Internal Server Error for unexpected errors or if `percentage_return` is unavailable in history points (data integrity issue). Logs request/response at INFO level including ticker, asset_type, start_date, end_date, and granularity parameters.
- `get_reference_portfolio_cache(request: Request)`: Dependency function that retrieves the reference portfolio cache from application state. Returns cachetools.FIFOCache(maxsize=5) instance. Raises HTTPException with 500 status if cache is not available. Used to provide the cache to the reference performance endpoint.
...
- `get_portfolio_allocation_endpoint(...) -> PortfolioAllocationResponse`: GET endpoint at `/portfolio/allocation` that requires JWT authentication. Supports filtering via query parameters: `asset_types` (optional, comma-separated list of asset types, e.g., "Stock,ETF") and `tickers` (optional, comma-separated list of ticker symbols, e.g., "AAPL,GOOG"). Verifies token via auth module, retrieves CompositePortfolio, PriceService, and AssetService from application state, parses comma-separated query parameters into lists (trims whitespace, filters empty strings), calls portfolio_service.get_portfolio_allocation() with composite, price_service, asset_service, asset_types, and tickers to fetch filtered positions with metadata, then returns PortfolioAllocationResponse containing list of AllocationPosition objects (each includes all Position fields plus optional metadata). Filtering uses OR logic: assets are included if they match any specified asset_type OR any specified ticker. Allocations are automatically recalculated against the filtered asset list only (sum to 100% of filtered assets) by the wpm library. Returns 400 Bad Request for invalid parameter format, 500 Internal Server Error if portfolio or services are unavailable. Logs request/response at INFO level including filter parameters.
...
- `get_portfolio_performance(portfolio: CompositePortfolio, price_service: PriceService, start_date: date, end_date: date) -> List[PortfolioHistoryPoint]`: Retrieves historical performance data from the wpm historical portfolio using `wpm.portfolio.get_historical_performance(portfolio, price_service, start_date, end_date)` which returns a list of PortfolioHistoryPoint objects. Transforms wpm PortfolioHistoryPoint objects (containing date, total_market_value, asset_positions, prices, percentage_return) into API PortfolioHistoryPoint models. Extracts date and converts to ISO format string (YYYY-MM-DD), extracts total_market_value as float, extracts asset_positions as Dict[str, float] mapping ticker to position value, extracts prices as Dict[str, float] mapping ticker to price, extracts percentage_return as float (validates it's not None and raises ValueError if unavailable). Logs function entry/exit and wpm library calls at INFO level. Returns list of API PortfolioHistoryPoint objects. Note: This function is kept for backward compatibility but may not be used after caching implementation.
- `get_reference_portfolio_performance(portfolio: CompositePortfolio, ticker: str, asset_type: str, price_service: PriceService, start_date: date, end_date: date, reference_portfolio_cache: Optional[cachetools.FIFOCache] = None, reference_portfolio_cache_lock: Optional[threading.Lock] = None) -> List[PortfolioHistoryPoint]`: Retrieves historical performance data for a reference portfolio based on a single ticker asset. Cache key is (ticker, asset_type). If reference_portfolio_cache is provided: acquires lock if present, looks up cache.get(key); on hit releases lock and calls get_historical_performance(cached_portfolio, ...) then returns transformed result; on miss creates reference portfolio, with lock sets cache[key] = reference_portfolio (FIFOCache evicts oldest when at maxsize 5), releases lock, calls get_historical_performance and returns. If cache is None, always creates reference portfolio and does not cache. Creation path: creates `Asset(ticker=ticker, asset_type=asset_type)`, `BuyAndHoldStrategy(reference_asset=reference_asset)`, `CurrencyService()`, calls `wpm.reference.portfolio.create_reference_portfolio(portfolio, strategy, price_service, currency_service)` to create reference portfolio, then `wpm.portfolio.get_historical_performance(reference_portfolio, price_service, start_date, end_date)`. Transforms wpm PortfolioHistoryPoint objects into API PortfolioHistoryPoint models (date to ISO string, total_market_value as float, asset_positions, prices, percentage_return validated not None). Logs at INFO level (cache hit/miss, reference portfolio creation). Returns list of API PortfolioHistoryPoint objects. Raises ValueError if reference portfolio creation fails or percentage_return is None in any history point.
...
- `get_portfolio_allocation(composite: CompositePortfolio, price_service: PriceService, asset_service: AssetService, asset_types: Optional[List[str]] = None, tickers: Optional[List[str]] = None) -> List[AllocationPosition]`: Retrieves filtered portfolio positions with metadata for the allocation endpoint. Fetches current prices using `wpm.portfolio.fetch_price_map(composite, price_service)` which returns `Dict[Asset, Optional[float]]`. Calls `wpm.portfolio.get_positions_with_allocations(composite, price_map, asset_types=asset_types, asset_tickers=tickers)` which returns `Dict[Asset, Tuple[Position, Decimal]]` mapping Asset to tuple of (Position, allocation_percentage). The wpm library handles filtering with OR logic (asset is included if it matches any specified asset_type OR any specified ticker) and automatically recalculates allocations against the filtered asset list only. Transforms wpm Position objects into API AllocationPosition models, extracting allocation_percentage from the tuple and converting Decimal to float. Calculates market_value and unrealized_gain_loss when prices are available. Calls `composite.get_asset_realized_pnl(asset.ticker)` for each position to get realized_gain_loss. Extracts filtered tickers from the positions and groups them by asset_type (since `get_metadata_batch()` requires all tickers to have the same asset_type). For each asset_type group, calls `asset_service.get_metadata_batch(tickers, asset_type)` which returns `Dict[str, Optional[Dict[str, Any]]]` mapping ticker to metadata dict (None if retrieval fails). Merges metadata into AllocationPosition objects. Handles None metadata gracefully. Logs function entry/exit, filtering parameters, wpm library calls, and metadata retrieval at INFO level. Returns list of AllocationPosition objects with metadata included.
...
**Key Functions**:
- `run_startup_logic(app: FastAPI, settings: Settings) -> None`: Executes application startup logic. Creates PriceService instance and stores it in `app.state.price_service`. Creates AssetService instance with `AssetService(price_service=price_service)` and stores it in `app.state.asset_service` for use by metadata endpoints. Imports CSV files using `wpm.importer.import_csv_files(import_dir)` and stores the returned CompositePortfolio in `app.state.composite_portfolio`. Creates a cloned historical portfolio using `composite_portfolio.clone()` (with no date filters to preserve full history) and stores it in `app.state.historical_portfolio` for use by the performance endpoint. Initializes `app.state.reference_portfolio_cache` using `cachetools.FIFOCache(maxsize=5)` for on-demand caching of reference portfolios by (ticker, asset_type), and optionally `app.state.reference_portfolio_cache_lock = threading.Lock()` for thread-safe access. Assumes `historical_portfolio.start_date` must exist (it is always set for such portfolios). If `start_date` is None, treats as a sanity error (logs at ERROR level, sets empty cache, continues startup). If `start_date` exists, calculates history points using `wpm.portfolio.get_historical_performance(historical_portfolio, price_service, start_date, date.today())`, transforms wpm PortfolioHistoryPoint objects (containing date, total_market_value, asset_positions, prices, percentage_return) to API PortfolioHistoryPoint models by extracting date (converted to ISO format string), total_market_value (as float), asset_positions (as Dict[str, float]), prices (as Dict[str, float]), and percentage_return (as float, validates it's not None and raises ValueError if unavailable), and stores them in `app.state.performance_cache` as a dictionary keyed by ISO date string (YYYY-MM-DD). Stores `date.today()` in `app.state.performance_cache_end_date` for validation. Logs cache creation at INFO level (number of points, date range). Generates OpenAPI specification using `generate_openapi_spec(app)`. Logs all operations at INFO level (ERROR level for sanity errors).
...
- `app.state.reference_portfolio_cache`: cachetools.FIFOCache(maxsize=5) for reference portfolios, keyed by (ticker, asset_type), populated on-demand
- `app.state.reference_portfolio_cache_lock`: Optional threading.Lock for thread-safe access to reference_portfolio_cache
...
- `InteractiveCLI.portfolio_all_command() -> None`: Calls GET `/portfolio/all` endpoint via TestClient with stored JWT token in Authorization header. Displays portfolio totals prominently including Total Cost Basis, Total Market Value, Total Unrealized P&L, and Total Realized P&L (formatted with sign for gains/losses, as currency with 2 decimal places). Then displays the response in formatted JSON using `json.dumps()` with `indent=2`. Handles authentication errors by prompting user to login if token is missing or expired.
...
- `InteractiveCLI.performance_command(end_date: Optional[str] = None, granularity: Optional[str] = None) -> None`: Calls GET `/portfolio/all/performance` endpoint via TestClient with stored JWT token in Authorization header. If `end_date` is None or not provided, defaults to today's date using `date.today().isoformat()`. Always includes `end_date` as a query parameter and includes optional `granularity` (`daily`, `weekly`, or `monthly`) when provided. Extracts `history_points` from the response, displays only the last 10 history points in formatted JSON using `json.dumps()` with `indent=2`, and prints a note when more than 10 points exist. Handles authentication errors (401), validation errors (400), and server errors (500) with user-friendly error messages.
- `InteractiveCLI.reference_performance_command(ticker: str, asset_type: str, end_date: Optional[str] = None, granularity: Optional[str] = None) -> None`: Calls GET `/reference/{ticker}/performance` endpoint via TestClient with stored JWT token in Authorization header. The reference portfolio uses the imported composite portfolio as the source of trades and aligns its start date with the original portfolio by relying on the endpoint defaults and not sending `start_date`. The reference ticker does **not** need to exist in the portfolio; instead, the CLI passes both `ticker` and `asset_type` as query parameters, and the backend constructs a reference `Asset` from these values. If `end_date` is None or not provided, the command defaults to today's date using `date.today().isoformat()`. Always includes `asset_type` and `end_date` as query parameters and includes optional `granularity` (`daily`, `weekly`, or `monthly`) when provided. Extracts `history_points` from the response, displays only the last 10 history points in formatted JSON using `json.dumps()` with `indent=2`, and prints a note when more than 10 points exist. Handles authentication errors (401), validation errors (400), and server errors (500) with user-friendly error messages.
...
- `performance [YYYY-MM-DD] [granularity]`: Retrieves historical performance data via `/portfolio/all/performance` endpoint, displays last 10 history points in formatted JSON. The first optional argument is interpreted as an end date when it is a valid ISO date (`YYYY-MM-DD`); otherwise it is treated as a `granularity` value (`daily`, `weekly`, or `monthly`). When no date is provided, defaults the end date to today. When granularity is omitted, the API default (`daily`) is used.
- `ref <ticker> <asset_type> [YYYY-MM-DD] [granularity]`: Retrieves performance for a reference portfolio based on a single reference asset defined by `ticker` and `asset_type` via `/reference/{ticker}/performance` endpoint. The reference portfolio uses the imported composite portfolio as the original and aligns its start date with the portfolio start by relying on the endpoint defaults. The reference ticker does not need to exist in the portfolio; it is used purely as a benchmark for returns comparison. The first optional argument after `<asset_type>` is interpreted as an end date when it is a valid ISO date (`YYYY-MM-DD`); otherwise it is treated as a `granularity` value (`daily`, `weekly`, or `monthly`). When no date is provided, the end date defaults to today. When granularity is omitted, the API default (`daily`) is used. Displays the last 10 history points in formatted JSON.
...
- `realized_gain_loss` (float, optional)
  - Description: Realized gain or loss on the position in USD
  - Source: `composite.get_asset_realized_pnl(ticker)` from wpm library
  - Calculation: Aggregates realized P/L from all lots' matched sells for this asset (handled by wpm library)
...
- `total_realized_gain_loss` (float, optional)
  - Description: Total realized gain or loss across all positions in the portfolio in USD
  - Source: `composite_portfolio.get_total_realized_pnl()` from wpm library
  - Calculation: Aggregates realized P/L from all sub-portfolios (handled by wpm library)
  - Validation: Can be negative (loss) or positive (gain)
  - Example: 1250.0 or -500.0
...
Response model for the `/portfolio/trades/{ticker}/all` endpoint. Contains all trades for a specific asset ticker without pagination.

**Fields**:
- `trades` (list[Trade], required)
  - Description: List of all trades (no pagination)
  - Validation: List of valid Trade objects
  - Example: [Trade(...), Trade(...), ...]

**Note**: The trades are filtered by the provided date range (if specified) and sorted according to the sort_by and sort_order parameters. All matching trades are returned without pagination.
...
- `percentage_return` (float, required)
  - Description: Percentage return from start_date to this date (lot-based return)
  - Source: `history_point.percentage_return` from wpm PortfolioHistoryPoint object
  - Validation: Float value (must not be None)
  - Example: 15.5
  - Note: If percentage_return is None in the wpm library history point, a ValueError is raised during transformation

### PortfolioPerformanceResponse
**Location**: `wpm_backend/models/portfolio.py`

Response model for the `/portfolio/all/performance` and `/reference/{ticker}/performance` endpoints. Contains a list of portfolio history points representing day-to-day historical performance.
...
Model representing a single portfolio position with metadata for the allocation endpoint. Extends the Position model with an optional metadata field.

**Fields**:
- All fields from `Position` model (ticker, asset_type, quantity, average_price, cost_basis, cost_basis_method, current_price, market_value, unrealized_gain_loss, allocation_percentage, realized_gain_loss)
- `metadata` (Optional[Dict[str, Any]], optional)
  - Description: Asset metadata dictionary containing information such as sector, industry, country, market_cap, category, etc. if available
  - Source: `asset_service.get_metadata_batch(tickers, asset_type)` from wpm library
  - Validation: Dictionary with string keys and Any values, or None if metadata retrieval fails
  - Example: {"name": "Apple Inc.", "sector": "Technology", "industry": "Consumer Electronics", "country": "United States", "market_cap": 3000000000000.0, "category": "Stock"} or None

**Note**: The allocation_percentage field represents the percentage allocation of this asset within the filtered asset list (not the entire portfolio). When filters are applied, allocations are automatically recalculated by the wpm library to sum to 100% of the filtered assets only.

### PortfolioAllocationResponse
**Location**: `wpm_backend/models/portfolio.py`

Response model for the `/portfolio/allocation` endpoint. Contains a list of filtered portfolio positions with metadata.

**Fields**:
- `assets` (List[AllocationPosition], required)
  - Description: List of filtered portfolio positions with metadata, ordered as returned by the wpm library
  - Source: Transformed from `get_positions_with_allocations()` result with metadata merged from `asset_service.get_metadata_batch()`
  - Validation: List of valid AllocationPosition objects
  - Example: [AllocationPosition(ticker="AAPL", asset_type="Stock", quantity=100.0, ..., metadata={"sector": "Technology", ...}), ...]

**Note**: The assets list contains only positions that match the filter criteria (if filters are provided). Filtering uses OR logic: assets are included if they match any specified asset_type OR any specified ticker. Allocations are calculated against the filtered asset list only and sum to 100% of filtered assets.
...
- **wpm** (git+https://github.com/waigore/wpmv2.git@dev): Wealth Portfolio Manager library installed from GitHub dev branch. Key components:
  ...
  - `wpm.portfolio.get_positions_with_allocations(portfolio: Portfolio, prices: Dict[Asset, Optional[float]], asset_types: Optional[List[str]] = None, asset_tickers: Optional[List[str]] = None) -> Dict[Asset, Tuple[Position, Decimal]]`: Get positions and their percentage allocations combined in a single dictionary. Supports optional filtering by `asset_types` (list of asset type strings) and `asset_tickers` (list of ticker strings) with OR logic (asset is included if it matches any specified asset_type OR any specified ticker). When filters are provided, allocations are automatically recalculated against the filtered asset list only (sum to 100% of filtered assets). Returns dictionary mapping Asset to tuple of (Position, allocation_percentage).
  ...
  - `CompositePortfolio.get_total_realized_pnl() -> float`: Calculate total realized profit/loss aggregated from sub-portfolios. Derives from lots' realized P/L. Returns total realized profit/loss in USD.
  - `CompositePortfolio.get_asset_realized_pnl(ticker: str, brokers: Optional[List[str]] = None) -> float`: Calculate realized profit/loss for a specific asset position. Aggregates realized P/L from all sub-portfolios. Returns realized profit/loss in USD for the specified ticker.
