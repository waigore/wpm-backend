...
- `get_reference_performance_endpoint(ticker: str, ...) -> PortfolioPerformanceResponse`: GET endpoint at `/reference/{ticker}/performance` that requires JWT authentication. Creates a reference portfolio using BuyAndHoldStrategy that converts all trades in the original portfolio to trades in a single reference asset defined by the requested `ticker` and `asset_type`, then retrieves historical performance. The reference ticker **does not need** to be present in the imported portfolio; it is used purely as a benchmark for comparison. Supports filtering and configuration via query parameters:
  ...
  - `start_date` (optional, ISO format YYYY-MM-DD, defaults to portfolio start_date): Start date for performance tracking. Parsed using the same rules as the portfolio performance endpoint.
...
- `get_reference_portfolio_performance(portfolio: CompositePortfolio, ticker: str, price_service: PriceService, start_date: date, end_date: date) -> List[PortfolioHistoryPoint]`: Retrieves historical performance data for a reference portfolio based on a single ticker asset. Validates ticker exists in portfolio using `portfolio.get_assets()` which returns `Dict[str, Asset]` mapping ticker to Asset object, extracts asset_type from the Asset object. Creates `Asset(ticker=ticker, asset_type=asset_type)` object, creates `BuyAndHoldStrategy(reference_asset=asset)` instance, creates `CurrencyService()` instance, calls `wpm.reference.portfolio.create_reference_portfolio(portfolio, strategy, price_service, currency_service)` to create reference portfolio, then calls `wpm.portfolio.get_historical_performance(reference_portfolio, price_service, start_date, end_date)` which returns a list of PortfolioHistoryPoint objects. Transforms wpm PortfolioHistoryPoint objects (containing date, total_market_value, asset_positions, prices, percentage_return) into API PortfolioHistoryPoint models. Extracts date and converts to ISO format string (YYYY-MM-DD), extracts total_market_value as float, extracts asset_positions as Dict[str, float] mapping ticker to position value, extracts prices as Dict[str, float] mapping ticker to price, extracts percentage_return as float (validates it's not None and raises ValueError if unavailable). Logs function entry/exit, ticker validation, reference portfolio creation, and wpm library calls at INFO level. Returns list of API PortfolioHistoryPoint objects. Raises ValueError if ticker is not found in portfolio, reference portfolio creation fails, or percentage_return is None in any history point.
